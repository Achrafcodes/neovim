...
-- Alternative: Format on save (add this to bottom of file)
...
-- Ctrl + ` to toggle terminal (like VSCode)
map({"n", "t"}, "<C-`>", function()
  -- Get current buffer type
  local buftype = vim.bo.buftype
  
  if buftype == "terminal" then
    -- If we're in any terminal, close it
    vim.cmd("close")
  else
    -- If we're not in terminal, toggle the main terminal
    require("nvchad.term").toggle { pos = "sp", id = "htoggleTerm", size = 0.3 }
  end
end, { desc = "Toggle/close terminal with Ctrl+`" })
...
map("n", "<leader>rl", function()
  ...
  if ext == "html" then
    ...
    print("Live server: http://localhost:8000/" .. file)
  elseif ext == "js" then
    local js_file = file
    local html_file = dir .. "/_temp_run.html"
